...
final class swift_ioc_containerTests: XCTestCase {
    ...
    class B : PIoCTestB {
        init(_ a : PIoCTestA = IoC.resolveOrNil()!) {
    ...
    func test_resolveOrNil_Should_Return_Nil_If_Nothing_Is_Registered() {
        XCTAssertNil(
            IoC.resolveOrNil(PIoCTestProtocol.self)
        )
    }
    ...
    func test_resolve_Should_ResolveOrNil_Registered_Singleton() throws{
        ...
        try IoC.registerSingleton(PIoCTestProtocol.self, testObject)
        let result = IoC.resolveOrNil(PIoCTestProtocol.self);
    ...
    func test_resolve_Should_Resolve_Registered_Singleton() throws{
        ...
        try IoC.registerSingleton(PIoCTestProtocol.self, testObject)
        let result = try IoC.resolve(PIoCTestProtocol.self);
    ...
    func test_resolve_Should_Resolve_Registered_Singleton_implicitly() throws{
        ...
        try IoC.registerSingleton(PIoCTestProtocol.self, testObject)
        let result:PIoCTestProtocol = try IoC.resolve();
    ...
    func test_resolve_Should_ResolveOrNil_Registered_Singleton_implicitly() throws{
        ...
        try IoC.registerSingleton(PIoCTestProtocol.self, testObject)
        let result:PIoCTestProtocol? = IoC.resolveOrNil();
    ...
    func test_registerSingleton_always_returns_the_same_instance(){
        ...
        try! IoC.registerSingleton(PIoCTestProtocol.self, testSingleton);
    ...
    func test_registerLazySingleton_always_returns_the_same_instance() throws{
        ...
        IoC.registerLazySingleton(PIoCTestProtocol.self, {() -> AnyObject in testSingleton})
    ...
    func test_registerSingleton_removes_old_lazy_registration() throws{
        ...
        IoC.registerLazySingleton(PIoCTestProtocol.self, {testSingletonOld})
        try IoC.registerSingleton(PIoCTestProtocol.self, testSingletonNew)
    ...
    func test_registerType_causes_resolve_to_always_create_a_new_instance() throws{
        IoC.registerType(PIoCTestProtocol.self, {()->AnyObject in CounterIoCTestClass()})

        let result1 = try IoC.resolve(PIoCTestProtocol.self) as? CounterIoCTestClass
        let result2 = try IoC.resolve(PIoCTestProtocol.self) as? CounterIoCTestClass
    ...
    func test_registerSingleton_should_throw_error_when_registered_object_doesent_confirm_to_protocol(){
        assertError(
            {()->Void in
                try IoC.registerSingleton(PIoCTestProtocol.self, A())
            },
    ...
    func test_resolve_should_fail_when_registerLazySingleton_is_called_with_incompatible_types() throws{
        IoC.registerLazySingleton(PIoCTestProtocol.self, {()->AnyObject in A()})

        assertResolveError(PIoCTestProtocol.self)
    }

    func test_resolve_should_fail_when_registerType_is_called_with_incompatible_types() throws{
        IoC.registerType(PIoCTestProtocol.self, {()->AnyObject in A()})

        assertResolveError(PIoCTestProtocol.self)
    }

    func test_unregisterAll_removes_registrations() throws{
        IoC.registerType(PIoCTestProtocol.self, {()->AnyObject in IoCTestClass()})
        try IoC.registerSingleton(PIoCTestProtocol2.self, IoCTestClass2())
        IoC.registerLazySingleton(PIoCTestProtocol3.self, {()->AnyObject in IoCTestClass3()})
    ...
    func test_constructType() throws{
        IoC.registerLazySingleton(PIoCTestA.self, {()->AnyObject in A()})
        IoC.registerLazySingleton(PIoCTestB.self, {()->AnyObject in B()})

        let result = try IoC.resolve(PIoCTestB.self)
    ...
    func assertResolveError<T>(_ interface: T.Type){
        assertError(
            {()->Void in
                _ = try IoC.resolve(T.self)
            },
            predicate: {(e)->Bool in e is IoCError})
    }

    func assertSingletonIsRegistered<T>(_ testSingleton: AnyObject, type: T.Type, file:StaticString = #file, line: UInt = #line){
        XCTAssertTrue(testSingleton === IoC.resolveOrNil(type) as AnyObject, "different singleton instance than expected", file: file, line: line)
        XCTAssertTrue(testSingleton === IoC.resolveOrNil(type) as AnyObject, "different singleton instance than expected", file: file, line: line)
        XCTAssertTrue(testSingleton === (try! IoC.resolve(type)) as AnyObject, "different singleton instance than expected", file: file, line: line)
        XCTAssertTrue(testSingleton === (try! IoC.resolve(type)) as AnyObject, "different singleton instance than expected", file: file, line: line)
    }
